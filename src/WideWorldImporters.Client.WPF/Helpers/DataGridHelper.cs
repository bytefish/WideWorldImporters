...
namespace WideWorldImporters.Client.WPF.Helpers
{
    ...
    public static class DataGridHelper
    {
        ...
        public static readonly DependencyProperty FixBindingGroupValidationErrorsForProperty =
            DependencyProperty.RegisterAttached("FixBindingGroupValidationErrorsFor", typeof(DependencyObject), typeof(DataGridHelper),
        ...
        private static void OnFixBindingGroupValidationErrorsForChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (oldobj != null)
            {
                ...
                var leftOverErrors = group.ValidationErrors != null ? Validation.GetErrors(group.Owner).Except(group.ValidationErrors).ToArray() : Validation.GetErrors(group.Owner).ToArray();
                ...
                foreach (var error in leftOverErrors)
                {
                    ...
                    if (error.BindingInError is BindingExpressionBase binding
                        && (binding.Target == null || TreeHelper.IsDescendantOf(binding.Target, oldobj))
                        && binding.BindingGroup == null
                        && (binding.ValidationErrors == null || binding.ValidationErrors.Count == 0 || !binding.ValidationErrors.Contains(error)))
                    {
                        typeof(Validation)
                            ?.GetMethod("RemoveValidationError", BindingFlags.Static | BindingFlags.NonPublic)
                            ?.Invoke(null, new object[] { error, group.Owner, group.NotifyOnValidationError });
                    }
                }
            }
        }
